// ─── Asset Imports ───────────────────────────────────────────────────────────
import integramindsImg from '../../asset/integraminds.webp';
import digitaltrackImg from '../../asset/digitaltrack.webp';
import seamlessImg from '../../asset/seamless.webp';
import transoftImg from '../../asset/transoft.webp';
import w3gradsImg from '../../asset/w3gradshd.webp';
import glaLogoImg from '../../asset/gla-logo.webp';

export const glaLogo = glaLogoImg;

// ─── Regional Contest Directors ───────────────────────────────────────────────
export const directors = [
  {
    country: 'Afghanistan',
    site: 'Kabul',
    contestName: 'The 2026 ICPC Asia Kabul Regional Contest',
    director: 'Dr. Mohammad Shah Omid',
    email: 'm.shah.omid@kateb.edu.af',
    institution: 'Kateb University, Kabul',
    website: 'http://icpc.kateb.edu.af/',
    mode: 'Onsite Contest',
    expectedDate: '22nd- 23rd October, 2026',
  },
  {
    country: 'Bangladesh',
    site: 'Dhaka',
    contestName: 'The 2026 ICPC Asia Dhaka Regional Contest',
    director: 'Dr MD Rashedul Islam',
    email: 'icpc@iubat.edu',
    institution: 'International University of Business Agriculture and Technology, Dhaka',
    website: 'https://icpc2026.iubat.edu/',
    mode: 'Online followed by Onsite Contest',
    expectedDate: '18th-19th December 2026',
  },
  {
    country: 'India',
    site: 'Amritapuri',
    contestName: 'The 2026 ICPC Asia Amritapuri Regional Contest',
    director: 'Dr. Vipin Pavithran',
    email: 'icpc@am.amrita.edu',
    institution: 'Amrita University, Amritapuri',
    website: 'https://amritaicpc.in/',
    mode: 'Online followed by Onsite Contest',
    expectedDate: 'End December 2026',
  },
  {
    country: 'India',
    site: 'Chennai',
    contestName: 'The 2026 ICPC Asia Chennai Regional Contest',
    director: 'Dr. John Deva Prasanna D S',
    email: 'johndevd@srmist.edu.in',
    institution: 'SRM Institute of Science and Technology Chennai',
    website: 'https://www.srmist.edu.in/events/international-collegiate-programming-contest-icpc-chennai-regional-2026/',
    mode: 'Online followed by Onsite Contest',
    expectedDate: '10th-11th December, 2026',
  },
  {
    country: 'India',
    site: 'Kanpur',
    contestName: 'The 2026 ICPC Asia Kanpur Regional Contest',
    director: 'Dr. Sandesh Gupta',
    email: 'sandesh@csjmu.ac.in',
    institution: 'School of Engineering and Technology, CSJM University, Kanpur, IIT Gandhinagar and IIIT Pune',
    website: 'https://kanpur.indiaicpc.in/',
    mode: 'Online followed by Onsite Contest',
    expectedDate: '22nd -23rd December, 2026',
  },
  {
    country: 'India',
    site: 'Mathura',
    contestName: 'The 2026 ICPC Asia Mathura Regional Contest',
    director: 'Dr. Anoop Kumar Gupta, Dr. Rohit Agrawal',
    email: 'rohit.agrwal@gla.ac.in',
    institution: 'GLA University, Mathura',
    website: 'https://mathuraicpc.in/',
    mode: 'Online followed by Onsite Contest',
    expectedDate: '27th- 28th December, 2026',
  },
  {
    country: 'India',
    site: 'Greater Noida',
    contestName: 'The 2026 ICPC Asia West Continent Championship',
    director: 'Dr. Phalguni Gupta, Dr. Diwakar Bharadwaj',
    email: 'pg@gla.ac.in, diwakar.bharadwaj@gla.ac.in',
    institution: 'GLA University, Greater NOIDA, Delhi NCR',
    website: 'https://wc.indiaicpc.in/',
    mode: 'Onsite Contest (Multi-site)',
    expectedDate: '20th – 21st March, 2027',
  },
  {
    country: 'Iran',
    site: 'Tehran',
    contestName: 'The 2026 ICPC Asia Tehran Regional Contest',
    director: 'Dr. Hamid Zarrabi-Zadeh',
    email: 'zarrabi@sharif.edu',
    institution: 'Sharif University of Technology, Tehran',
    website: 'https://icpc.sharif.edu',
    mode: 'Onsite Contest',
    expectedDate: '24th-25th December 2026',
  },
  {
    country: 'Pakistan',
    site: 'Topi',
    contestName: 'The 2026 ICPC Asia Topi Regional Contest',
    director: 'Dr. Masroor Hussain',
    email: 'masroor.hussain@gmail.com',
    institution: 'Ghulam Ishaq Khan Institute of Engineering Sciences and Technology',
    website: 'https://www.giki.edu.pk/icpc',
    mode: 'Online followed by Onsite Contest',
    expectedDate: 'Mid December 2026',
  },
  {
    country: 'Sri Lanka',
    site: 'Peradeniya',
    contestName: 'The 2026 ICPC Asia Sri Lanka Regional Contest',
    director: 'Dr. Upul Jayasinghe',
    email: 'upuljm@eng.pdn.ac.lk',
    institution: 'University of Peradeniya, Peradeniya',
    website: 'https://icpc.ieee.lk/',
    mode: 'Online followed by Onsite Contest',
    expectedDate: '5th-6th December 2026',
  },
];

// ─── Important Dates ──────────────────────────────────────────────────────────
export const importantDates = [
  { country: 'Afghanistan', site: 'Kabul', mode: 'Onsite Contest', regDate: 'TBA', onlineDate: 'TBA', date: '22nd- 23rd October, 2026' },
  { country: 'Bangladesh', site: 'Dhaka', mode: 'Online followed by Onsite Contest', regDate: 'TBA', onlineDate: 'TBA', date: '18th-19th December 2026' },
  { country: 'India', site: 'Amritapuri', mode: 'Online followed by Onsite Contest', regDate: 'TBA', onlineDate: 'TBA', date: 'End December 2026' },
  { country: 'India', site: 'Chennai', mode: 'Online followed by Onsite Contest', regDate: '30th September 2026', onlineDate: '3rd October 2026', date: '10th-11th December, 2026' },
  { country: 'India', site: 'Kanpur', mode: 'Online followed by Onsite Contest', regDate: 'TBA', onlineDate: 'TBA', date: '22nd -23rd December, 2026' },
  { country: 'India', site: 'Mathura', mode: 'Online followed by Onsite Contest', regDate: 'TBA', onlineDate: 'TBA', date: '27th- 28th December, 2026' },
  { country: 'India', site: 'Greater Noida', mode: 'Onsite Contest (Multi-site)', regDate: 'TBA', onlineDate: 'TBA', date: '20th – 21st March, 2027' },
  { country: 'Iran', site: 'Tehran', mode: 'Onsite Contest', regDate: 'TBA', onlineDate: 'TBA', date: '24th-25th December 2026' },
  { country: 'Pakistan', site: 'Topi', mode: 'Online followed by Onsite Contest', regDate: 'TBA', onlineDate: 'TBA', date: 'Mid December 2026' },
  { country: 'Sri Lanka', site: 'Peradeniya', mode: 'Online followed by Onsite Contest', regDate: 'TBA', onlineDate: 'TBA', date: '5th-6th December 2026' },
];

// ─── Sidebar Links ────────────────────────────────────────────────────────────
export const sidebarLinks = [
  {
    href: 'https://icpcasiawest.vercel.app/DIRECTOR_REPORT.pdf',
    title: "DIRECTOR'S REPORT",
    subtitle: 'AWC 2025-2026',
    colorScheme: 'red',
    isNew: true,
  },
  {
    href: 'https://icpcasiawest.vercel.app/Anoun2026.pdf',
    title: 'Programme Schedule',
    subtitle: 'AWC 2025-2026',
    colorScheme: 'red',
    isNew: true,
  },
  {
    href: 'https://icpcasiawest.vercel.app/results?session=94014565-8795-4278-b03c-33187bea12ae',
    title: 'AWC Result',
    subtitle: 'Asia West Championship (2025-2026)',
    colorScheme: 'green',
    isNew: false,
  },
  {
    href: 'https://icpcasiawest.vercel.app/PROBLEM_SET26.pdf',
    title: 'Problem Set',
    subtitle: 'AWC Problem Set 2025-2026',
    colorScheme: 'red',
    isNew: true,
  },
  {
    href: 'https://icpcasiawest.vercel.app/teams?session=94014565-8795-4278-b03c-33187bea12ae',
    title: 'Selected Team for AWC 2025–26',
    subtitle: '',
    colorScheme: 'red',
    isNew: true,
  },
  {
    href: 'http://icpcasiawest.vercel.app/Asia-West_Rules.pdf',
    title: 'Contest Rules',
    subtitle: 'AWC 2025-2026',
    colorScheme: 'green',
    isNew: false,
  },
  {
    href: 'https://wc.indiaicpc.in/problem_set_2024_25.pdf',
    title: '🏆 AWC Problem Set (2024-25)',
    subtitle: 'AWC Problem Set (2024-25)',
    colorScheme: 'blue',
    isNew: false,
  },
  {
    href: 'https://icpcasiawest.vercel.app/teams?session=1f1672dc-34bd-4bb1-8e58-a751ba2ff12c',
    title: 'List of Selected Teams for Asia West Championship 2024-25',
    subtitle: '',
    colorScheme: 'gray',
    isNew: false,
  },
];

// ─── Team Selection Rules ─────────────────────────────────────────────────────
export const teamSelectionRules = [
  'Teams are invited to participate in the contest based on their performance in their respective regional contest sites.',
  'Only the top few teams from each site will be selected for the contest.',
  'Even though the Champion Team of each regional site—the WINNER of the site—gets a slot in the World Finals, all the winning teams of all regional sites of Asia West Continent must participate in the AWC Championship trophy.',
  'Other teams of the Champion Institutes (i.e., an institute that has at least one Champion Team) are not eligible to participate in AWC.',
  'Team composition, under no circumstances, can be changed.',
  'The number of teams to be selected from each site is based on the total number of teams that participated in the Online contest and the On-site contest. The weightage for online contest teams is 0.5 and for onsite contest teams is 1. Higher the value of the metric, the larger the number.',
  'However, the minimum number of teams from each site that will be eligible is 5.',
  'The RCD of each site will select the top few teams for the AWC Finals based on the ranking of their respective Regional Contest. There is no restriction on the number of teams from one institution (except for item 4 above).',
  'Contests of all sites are assumed to have the same degree of difficulty. In case of a tie, World Finals Rules will be followed (number of problems solved, penalty time, time of last problem solved, time of 2nd last problem solved, and so on).',
];

// ─── Gallery photos ───────────────────────────────────────────────────────────
export const galleryPhotos = [
  {
    alt: 'Campus ambience',
    style: 'absolute left-[3%] top-[20%] rotate-[-10deg] bg-white p-2 shadow-lg w-[42%] h-[48%]',
    src: 'https://placehold.co/400x300/e2e8f0/475569?text=Campus+Ambience',
  },
  {
    alt: 'Inaugural moment',
    style: 'absolute left-[30%] top-[2%] rotate-[6deg] bg-white p-2 shadow-lg w-[46%] h-[50%]',
    src: 'https://placehold.co/400x300/dbeafe/1e40af?text=Inaugural+Moment',
  },
  {
    alt: 'Main contest',
    style: 'absolute left-[54%] top-[24%] rotate-[10deg] bg-white p-2 shadow-lg w-[40%] h-[45%]',
    src: 'https://placehold.co/400x300/dcfce7/166534?text=Main+Contest',
  },
  {
    alt: 'Award ceremony',
    style: 'absolute left-[18%] top-[48%] rotate-[-2deg] bg-white p-2 shadow-lg w-[52%] h-[50%]',
    src: 'https://placehold.co/400x300/fef9c3/713f12?text=Award+Ceremony',
  },
];

// ─── Nav Links ────────────────────────────────────────────────────────────────
export const navLinks = [
  { label: 'Home', href: '/', hasDropdown: false },
  {
    label: 'Results',
    href: '/results',
    hasDropdown: true,
    dropdownItems: [
      { label: 'AWC 2025-26', href: '/results/awc-2025-26' },
      { label: 'AWC 2024-25', href: '/results/awc-2024-25' },
    ],
  },
  {
    label: 'Selected Teams',
    href: '/teams',
    hasDropdown: true,
    dropdownItems: [
      { label: 'AWC 2025-26', href: '/teams/awc-2025-26' },
      { label: 'AWC 2024-25', href: '/teams/awc-2024-25' },
    ],
  },
  { label: 'Steering Committee', href: '/committee', hasDropdown: false },
  { label: 'Organising Committee', href: '/organising-committee', hasDropdown: false },
  { label: 'Development Team', href: '/about', hasDropdown: false },
];

// ─── Footer Quick Links ───────────────────────────────────────────────────────
export const footerLinks = [
  { label: 'Home', href: '/' },
  { label: 'Results', href: '/results' },
  { label: 'Selected Teams', href: '/teams' },
  { label: 'Steering Committee', href: '/committee' },
  { label: 'About', href: '/about' },
];

// ─── Partner Logos ────────────────────────────────────────────────────────────
export const partnerLogos = [
  { alt: 'Integraminds logo', src: integramindsImg },
  { alt: 'Digital Track logo', src: digitaltrackImg },
  { alt: 'Seamless Infotech logo', src: seamlessImg },
  { alt: 'Transoft Infotech logo', src: transoftImg },
  { alt: 'W3grads logo', src: w3gradsImg },
];

// ─── Steering Committee ───────────────────────────────────────────────────────
export const steeringCommittee = [
  {
    name: "Dr. Anoop Kumar Gupta",
    affiliation: "GLA University, Mathura, India",
    email: "vc@gla.ac.in",
    url: "https://mathuraicpc.in/",
  },
  {
    name: "Dr. Rohit Agrawal",
    affiliation: "GLA University, Mathura, India",
    email: "rohit.agrwal@gla.ac.in",
    url: "https://mathuraicpc.in/",
  },
  {
    name: "Dr. Diwakar Bharadwaj",
    affiliation: "GLA University, Greater NOIDA, Delhi NCR, India",
    email: "diwakar.bharadwaj@gla.ac.in",
    url: "https://wc.indiaicpc.in/",
  },
  {
    name: "Dr. Vipin Pavithran",
    affiliation: "Amrita University, Amritapuri, India",
    email: "icpc@am.amrita.edu",
    url: "https://amritaicpc.in/",
  },
  {
    name: "Dr. John Deva Prasanna D S",
    affiliation: "SRM Institute of Science and Technology Chennai, India",
    email: "johndevd@srmist.edu.in",
    url: "https://www.srmist.edu.in/events/international-collegiate-programming-contest-icpc-chennai-regional-2026/",
  },
  {
    name: "Dr. Mohammad Shah Omid",
    affiliation: "Kateb University, Kabul, Afghanistan",
    email: "m.shah.omid@kateb.edu.af",
    url: "http://icpc.kateb.edu.af/",
  },
  {
    name: "Dr. Sandesh Gupta",
    affiliation: "School of Engineering and Technology, CSJM University, Kanpur, IIT Gandhinagar and IIIT Pune, India",
    email: "sandesh@csjmu.ac.in",
    url: "https://kanpur.indiaicpc.in/",
  },
  {
    name: "Dr. Hamid Zarrabi-Zadeh",
    affiliation: "Sharif University of Technology, Tehran, Iran",
    role: "Secretary & Associate Director ICPC Asia WC Super Region",
    email: "zarrabi@sharif.edu",
    url: "https://icpc.sharif.edu",
  },
  {
    name: "Dr. Masroor Hussain",
    affiliation: "Ghulam Ishaq Khan Institute of Engineering Sciences and Technology, Topi, Pakistan",
    role: "Regional Contest Director and Associate Director ICPC Asia WC Super Region",
    email: "masroor.hussain@gmail.com",
    url: "https://www.giki.edu.pk/icpc",
  },
  {
    name: "Dr. Upul Jayasinghe",
    affiliation: "University of Peradeniya, Peradeniya, Sri Lanka",
    email: "upuljm@eng.pdn.ac.lk",
    url: "https://icpc.ieee.lk/",
  },
  {
    name: "Dr. MD Rashedul Islam",
    affiliation: "International University of Business Agriculture and Technology, Dhaka, Bangladesh",
    email: "icpc@iubat.edu",
    url: "https://icpc2026.iubat.edu/",
  },
];
